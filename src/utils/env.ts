...
function createEnv<TServer extends ZodRawShape, TClient extends ZodRawShape>({
...
}: CreateEnvOptions<TServer, TClient>): z.infer<ZodObject<TServer>> &
	z.infer<ZodObject<TClient>> {
	if (skipValidation) {
		return runtimeEnv as z.infer<ZodObject<TServer>> &
			z.infer<ZodObject<TClient>>;
	}
	...
	if (typeof window === "undefined") {
		const serverEnv = Object.fromEntries(
			Object.keys(server).map((key) => [key, processedEnv[key]]),
		);
	...
	const clientEnv = Object.fromEntries(
		Object.keys(client).map((key) => [key, processedEnv[key]]),
	);
	...
	return new Proxy(processedEnv, {
		get(target, prop) {
			...
			if (typeof window !== "undefined" && prop in server) {
				throw new Error(
					`❌ Próba dostępu do zmiennej serwerowej "${prop}" po stronie klienta!`,
				);
			}
